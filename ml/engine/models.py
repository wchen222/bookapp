...
class MFModel(nn.Module):
    ...
    def predict_all_scores(self, user_idx):
        ...
        dot_product = torch.matmul(self.movie_embeddings.weight, user_vec)
    ...
    @torch.no_grad()
    def predict_from_user_vector(self, user_vector, bias_scale = 0.5):
        dot_products = torch.matmul(self.item_embeddings.weight, user_vector)
        raw_scores = self.global_bias + (self.movie_biases.weight.squeeze() * bias_scale) + dot_products
